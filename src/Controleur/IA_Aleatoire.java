...
public class IA_Aleatoire extends IA {
    ...
    public int calcul(Plateau p, int horizon,int minmax){
        ...
        Configuration.instance().logger().info(coups.size() + " !1! " + alea);
        c1 = coups.get(alea);
        Configuration.instance().logger().info(( (Mouvement) c1).toString());
        ...
        // Second coup avec pion déjà choisi
            ...
            alea = r.nextInt(coups.size());
            //Configuration.instance().logger().(info(coups.size() + " !2! " + alea);
            c2 = coups.get(alea);
            Configuration.instance().logger().info(( (Mouvement) c2).toString() + " -- ");
            //Configuration.instance().logger().(info(( (Mouvement) c2).arrivee().toString());
    ...
    void gestionFocus(int alea ){
        ...
        Configuration.instance().logger().info("Focus choisi Alea = " + Epoque.depuisIndice(alea));
